...
class RailFence: NSViewController {
    ...
    @IBAction func encodeBtn(_ sender: Any) {
        ...
        keywordField.stringValue = keywordField.stringValue.filter { return numerals.contains($0) }
        keyword = Int(keywordField.intValue)
        if keyword < 1 {
            dialogError(question: "Please, change the key!", text: "Error: Wrong key.")
            return
        }
